# Yogo Cloud EKS Monitoring Setup Guide

Welcome to Yogo Cloud! This guide explains how to deploy the `yogo-ai` MCP agent into your own AWS EKS cluster and securely connect it to Yogo Cloud.

Our architecture uses a **Push-Based, Zero-Trust model**. You do not need to grant us cross-account IAM roles or expose your cluster to the public internet. Instead, the `yogo-ai` agent establishes an outbound-only WebSocket tunnel back to Yogo Cloud.
...
The `yogo-ai` container images are securely hosted in Yogo Cloud's private Amazon Elastic Container Registry (ECR). 
...
Before deploying, you must provide your cluster's OIDC Issuer URL to your Yogo Cloud Administrator.

1. Retrieve your OIDC Issuer URL by running:
   ```bash
   aws eks describe-cluster --name <cluster-name> --query "cluster.identity.oidc.issuer" --output text
   ```
2. Send this URL to your Yogo Cloud Administrator.
...
1. **Configure your Secrets**: Copy `client-values.yaml.example` to `client-values.yaml` and open it in a text editor.
   - Insert the `socketToken` provided by the Yogo Cloud Administrator.
   - Insert the **IAM Role ARN** provided by your Yogo Cloud Administrator into the `serviceAccount.annotations` section.
...
**Option A: Using the raw chart directory (Recommended)**
```bash

helm install yogo-ai ./yogo-ai-stack -f client-values.yaml --namespace yogo-ai --create-namespace
```

**Option B: Using the `.tgz` package**
```bash

helm install yogo-ai ./yogo-ai-stack-0.67.0.tgz -f client-values.yaml --namespace yogo-ai --create-namespace
```

Once the chart is deployed, the Yogo-AI agent will automatically establish an outbound WebSocket connection to Yogo Cloud. It may take 2-3 minutes for the qdrant db to initialize and the agent to be ready.

---

## Step 3: Finalize the Connection
Please **log into your Yogo Cloud Dashboard**, navigate to the specific **Kubernetes Cluster Dashboard page**, the dashboard will securely connect to your AI Agent and you will be presented with the kubernetes dashboard of your cluster with yogo-ai agent ready to use.
